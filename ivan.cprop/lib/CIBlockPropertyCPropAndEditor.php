...
class CIBlockPropertyCPropAndEditor
{
    ...
    public function GetUserTypeDescription()
    {
        return array(
            'PROPERTY_TYPE' => 'S',
            'USER_TYPE' => 'CL',
            'DESCRIPTION' => Loc::getMessage('IEX_CPROP_EDITOR_DESC_HTML'),
    ...
    {
        $hideText = Loc::getMessage('IEX_CPROP_EDITOR_HIDE_TEXT');
        $clearText = Loc::getMessage('IEX_CPROP_EDITOR_CLEAR_TEXT');
        ...
        if(!empty($arProperty['USER_TYPE_SETTINGS'])){
            ...
            return '<span>'.Loc::getMessage('IEX_CPROP_EDITOR_ERROR_INCORRECT_SETTINGS').'</span>';
        }
        ...
        foreach ($arFields as $code => $arItem){
            ...
            else if($arItem['TYPE'] === 'customhtml'){
    ...
    {
        $btnAdd = Loc::getMessage('IEX_CPROP_EDITOR_SETTING_BTN_ADD');
        $settingsTitle =  Loc::getMessage('IEX_CPROP_EDITOR_SETTINGS_TITLE');
        ...
        $result = '<tr><td colspan="2" align="center">
            <table id="many-fields-table" class="many-fields-table internal">        
                <tr valign="top" class="heading mf-setting-title">
                   <td>XML_ID</td>
                   <td>'.Loc::getMessage('IEX_CPROP_EDITOR_SETTING_FIELD_TITLE').'</td>
                   <td>'.Loc::getMessage('IEX_CPROP_EDITOR_SETTING_FIELD_SORT').'</td>
                   <td>'.Loc::getMessage('IEX_CPROP_EDITOR_SETTING_FIELD_TYPE').'</td>
                </tr>';
                        ...
                        <td>
                            <table class="mf-img-table">
                                <tr>
                                    <td>'.$content.'<br>
                                        <div>
                                            <label><input name="'.$strHTMLControlName['VALUE'].'['.$code.'][DEL]" value="Y" type="checkbox"> '. Loc::getMessage("IEX_CPROP_EDITOR_FILE_DELETE") . '</label>
    ...
    {
        $showText = Loc::getMessage('IEX_CPROP_EDITOR_SHOW_TEXT');
        $hideText = Loc::getMessage('IEX_CPROP_EDITOR_HIDE_TEXT');
    ...
    private static function getOptionList($selected = 'string')
    {
        $result = '';
        $arOption = [
            'string' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_STRING'),
            'file' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_FILE'),
            'text' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_TEXT'),
            'date' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_DATE'),
            'element' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_ELEMENT'),
            'customhtml' => Loc::getMessage('IEX_CPROP_EDITOR_FIELD_TYPE_HTML')
        ];
